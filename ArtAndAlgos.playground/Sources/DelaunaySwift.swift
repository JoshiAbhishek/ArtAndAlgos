//  Delaunay Created by Alex Littlejohn and Modified by Abhishek Joshi
...
@objc open class Delaunay : NSObject {
    ...
    open func triangulate(_ vertices: [Vertex]) -> [Triangle] {
        ...
        let results = completed.flatMap { (circumCircle) -> Triangle? in
            ...
            let v1 = circumCircle.vertex1
            let v2 = circumCircle.vertex2
            let v3 = circumCircle.vertex3
            
            let sumX = (1/3)*(v1.x + v2.x + v3.x)
            let sumY = (1/3)*(v1.y + v2.y + v3.y)
            
            return Triangle(vertex1: v1, vertex2: v2, vertex3: v3, centroid: Vertex(x: (sumX), y: (sumY)))
        }
